...
Everything the [README](../README.md) leaves out: what each engine is good for,
how the optional GPU works, what the numbers mean, and how the pieces fit.
...
![Project page: classes, engine and export](project.jpg)
...
![Review page: boxes, handles, class legend](review.jpg)
...
![Settings page: optional GPU](settings.jpg)
...
GPU is still one dataset, and the benchmark below shows the two paths finding
...
period. A labeling run keeps four images in flight (`REMOTE_LABELER_CONCURRENCY`),
which is what `max_containers=4` in the recipe is sized for — raise the two
together or the extra containers never wake.
...
Creative Commons Attribution licenses — see [docs/DEMO_IMAGES.md](DEMO_IMAGES.md).
